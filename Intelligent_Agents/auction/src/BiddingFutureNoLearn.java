...
public class BiddingFutureNoLearn implements BiddingInt{
  ...
  private boolean oppWonOne;
	...
	public BiddingFutureNoLearn(Topology topology,Agent agent) {
    ...
    public double Bid(double mc,double mcOp, Task task,Random random, 
    	...
    	if (FirstAdvantage !=0) {
       	...
       	// 2. Bid a bit more then mc
       		...
       		bid = mcOp-mFuturProfitOp*Math.pow(1.05, task.id)*0.5;
       		bid = ratioBelow * bid;
       		if(bid<mc) {
       			bid=mc;
       			bid=bid*ratioAbove;
